...
class DB extends \PDO
{
	...
	{
		$file = CONFIG_DIR.'database.ini';
		if (! $settings = parse_ini_file ( $file, TRUE )) {
			throw new exception ( 'Unable to open ' . $file . '.' );
		}
		
		$dns = $settings [$this->database] ['driver'] . ':host=' . $settings [$this->database] ['host'] . ((! empty ( $settings [$this->database] ['port'] )) ? (';port=' . $settings [$this->database] ['port']) : '') . ';dbname=' . $settings [$this->database] ['schema'];
		
		$this->_link = parent::__construct ( $dns, $settings [$this->database] ['username'], $settings [$this->database] ['password'] );
	}
	...
	public function run($sql, $bind = '')
	{
		...
		try {
			...
			if (preg_match ( '/^(' . implode ( '|', array ('select', 'describe', 'pragma', 'with' ) ) . ') /i', $this->sql )) {
